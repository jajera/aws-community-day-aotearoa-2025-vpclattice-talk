data "aws_region" "current" {} # Get the current region

module "ecs_cluster_fargate" {
  source = "tfstack/ecs-cluster-fargate/aws"

  # Core Configuration
  cluster_name = "${var.name_prefix}3"
  suffix       = var.name_suffix

  # VPC Configuration
  vpc = {
    id = var.vpc_ids["app_2"]
    private_subnets = [
      for i, subnet in var.app_2_private_subnet_ids :
      { id = subnet, cidr = var.app_2_private_subnet_cidrs[i] }
    ]
    public_subnets = [
      for i, subnet in var.app_2_public_subnet_ids :
      { id = subnet, cidr = var.app_2_public_subnet_cidrs[i] }
    ]
  }
  ...
  # setup the analytics service to demonstrate the ECS Fargate service
  ecs_services = [
    {
...
EOF
      ...
      EOT
          ...
          logConfiguration = {
            logDriver = "awslogs"
            options = {
              awslogs-group         = "/aws/ecs/${var.name_prefix}3-analytics-service"
              awslogs-region        = data.aws_region.current.region
              awslogs-stream-prefix = "${var.name_prefix}3-analytics-service"
            }
          }
        }
      ])
      ...
      subnet_ids       = var.app_2_private_subnet_ids
      security_groups  = [var.security_group_ids["fargate_sg"]]
  ...
  ecs_autoscaling = [
    {
      service_name           = "${var.name_prefix}3-analytics-service"
      min_capacity           = 2
      max_capacity           = 4
  ...
  tags = {
    Name = "${var.name_prefix}3-ecs-fargate"
  }
}
