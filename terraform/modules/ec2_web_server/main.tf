...
module "ec2_web_server" {
  ...
  name = "${var.name_prefix}1-web-server"

  vpc_id                 = var.vpc_ids["client"]               # vpc id of the client VPC from the VPC module
  subnet_id              = var.ec2_client_public_subnet_ids[0] # create ec2 in the first public subnet of the client VPC
  vpc_security_group_ids = [var.security_group_ids["web_server_sg"]]

  instance_type    = "t3.small"
  assign_public_ip = true # assign a public IP to the EC2 web server

  enable_ssm = true # allows us to connect to the EC2 web server using SSM when we need to troubleshoot

  # setup the apache web server that runs the php web page to demonstrate the VPC Lattice microservices
  user_data = <<-EOT
  #!/bin/bash
  set -euo pipefail
  hostname ${var.name_prefix}1-web-server
