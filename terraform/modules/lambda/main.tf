############################################
# S3 Bucket, store the zip files for the lambda functions
############################################
...
module "s3_bucket" {
  ...
  bucket_suffix     = var.name_suffix
  enable_versioning = true
  force_destroy     = true # for development purposes, use with caution
  tags = {
    Environment = "dev"
    VPC         = "app-1"
  }
...
module "lambda_products_service" {
  ...
  function_name = "${var.name_prefix}2-products-service"
  ...
  vpc_config = {
    vpc_id             = var.vpc_ids["app_1"] # create lambda in app 1 VPC
    subnet_ids         = var.app_1_private_subnet_ids
    security_group_ids = [var.security_group_ids["lambda_sg"]]
  }
...
module "lambda_orders_service" {
  ...
  function_name = "${var.name_prefix}2-orders-service"
  handler       = "index.handler"
  runtime       = "nodejs20.x"
  timeout       = 30
...
module "lambda_payments_service" {
  ...
  function_name = "${var.name_prefix}2-payments-service"
  handler       = "index.handler"
  runtime       = "nodejs20.x"
  timeout       = 30
...
module "lambda_inventory_service" {
  ...
  function_name = "${var.name_prefix}2-inventory-service"
  handler       = "index.handler"
  runtime       = "nodejs20.x"
  timeout       = 30
...
module "lambda_analytics_service" {
  ...
  function_name = "${var.name_prefix}2-analytics-service"
  handler       = "index.handler"
  runtime       = "nodejs20.x"
  timeout       = 30
...
module "lambda_notfound_service" {
  ...
  function_name = "${var.name_prefix}2-notfound-service"
  handler       = "index.handler"
  runtime       = "nodejs20.x"
  timeout       = 30
...
module "lambda_products_service_v2" {
  ...
  function_name = "${var.name_prefix}2-products-service-v2"
  handler       = "index.handler"
  runtime       = "nodejs20.x"
  timeout       = 30
